...
Important: `bda update` in v0.10.9 updates command/session behavior, cleans Hermes BDA provider/model config, and keeps `bda/gpt-oss-20b-local` available as an optional model with a Hermes-safe context declaration. Restart Hermes Desktop after the update so the model picker reloads without the legacy BDA group. If Hermes still shows two BDA groups, close Hermes, run `bda config-clean`, and open Hermes again.
