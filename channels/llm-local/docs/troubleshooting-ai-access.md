...
| **คำตอบแปลกๆ มี `<function=...>` โผล่เป็นข้อความ / tool ไม่ทำงาน** (Claude Code) | เคยเกิดจาก backend เก่า — แก้ถาวรแล้ว (2026-07-06: ทุก node เป็น Qwen3.6+qwen3_xml) | ถ้ายังเจอ = แจ้ง admin ทันที |
| **Claude Code ขึ้น 400 "Invalid model name ... claude-sonnet-4-5"** | ชื่อ model เก่าถูกปลดออกจากระบบ (2026-07-06) | เปลี่ยน model ใน config เป็น **`claude-code-local`** (ชื่อเดียวที่ใช้ ทั้ง Desktop และ CLI) |
| **AI ตอบเฉยๆ ไม่ลงมือทำ (งานเล็กมาก)** | โมเดลคิดว่าง่ายพอ "ทำในหัว" ไม่เรียก tool | สั่งชัดในโจทย์: "ใช้ tool เขียนไฟล์/รันคำสั่งจริง" |
| **AI บอกเสร็จแต่ไม่มีของ** | 2 แบบ: โม้จริง (test 0 ตัวแต่บอก complete) หรือ **ทำจริงแต่ผิดโฟลเดอร์** (cwd drift — เจอจริง: งานครบ 10/10 อยู่คนละ dir) | `find` หาผลงานทั่วก่อน แล้วเช็ค test ผ่าน + `git log` ด้วยตาตัวเอง — ดู `channels/llm-local/docs/ai-employee-manual.md` |
...
| **"Invalid API Response / empty or unparsable" นานๆ ครั้ง** (Cline/IDE) | transient — client ส่วนใหญ่ retry เองสำเร็จ | ปล่อยให้ retry; **ถ้าเจอถี่ (หลายครั้งใน 1 งาน) ให้แจ้ง admin พร้อมเวลาเกิด** |
| **ข้อความไทยมีอักขระเพี้ยนบางจุด** (เช่น ตัวอักษรแปลกปน) | known limitation ของโมเดล quantized — โค้ด/ผลเทสต์ไม่กระทบ | ไม่ต้องแจ้ง ถ้ารำคาญให้สั่งงานเป็น docstring อังกฤษ |
...
- Historical note: `claude-sonnet-4-5` ถูกปลดจากระบบเมื่อ 2026-07-06; ตอนนี้ใช้ชื่อเดียว `claude-code-local`. บันทึกเดิมระบุว่าโมเดลนี้ต้องอยู่ GX10 เท่านั้น (A40 leak tool format กับ request ใหญ่) และเคยคุมโดย litellm alias (gate anthropic lane rewrite ปิดด้วย `BDA_ANTHROPIC_LANE_REWRITE_MODELS=disabled`)
