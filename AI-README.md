# AI Entry Point: AKS AI Dev Standard (เดิม BDA AI Dev Standard)

คุณคือผู้ช่วยทำงานของ BDA ให้ทำงานแบบมืออาชีพ กระชับ ตรวจสอบได้ และไม่สร้างหลักฐานปลอม

Layout ตั้งแต่ v1.1.0: เนื้อหากลางทุกช่องทางอยู่ใน `core/` (`core/commands/`, `core/workflows/`, `core/policies/`, `core/checklists/`, `core/templates/`, `core/prompts/`); ของเฉพาะช่องทางอยู่ใน `channels/llm-local/` (Hermes/Claude Code/Cline/IDE + `aks`/`bda` CLI) และ `channels/thclaws/` (กล่องบน GX10 — ไม่มี aks/bda CLI, Hermes, slash commands)

## Copy this into AI

```text
ใช้ AKS AI Dev Standard เป็นมาตรฐานการทำงาน:
1) อ่าน task brief และ context ก่อน
2) ถ้าเป็น repo ให้สำรวจไฟล์/โครงสร้างก่อนแก้
3) ถ้างานผูกกับ Obsidian ให้หา `00-Agent-Context.md` หรือ `.bda/obsidian-context.md`; ถ้ายังไม่มีให้ใช้ `core/commands/init.md`
...
```text
ใช้ AKS AI Dev Standard แบบสั้น:
...
- ไม่รู้ requirement: `core/commands/understand-task.md`
- ต้องวางแผน: `core/commands/plan-work.md`
- init Obsidian/project context: `core/commands/init.md`
- feature: `core/commands/build-feature.md`
- bug: `core/commands/fix-bug.md`
- review: `core/commands/review-change.md`
- docs/writing: `core/commands/write-document.md`
- Obsidian: `core/commands/update-obsidian.md`
- QA/product evidence พร้อม screenshot report: `core/commands/test-report.md`, `core/commands/test-scenario-report.md`, `core/workflows/test-scenario-report.md`, `core/templates/test-scenario-report.md`
- feedback เพื่อปรับปรุง AKS AI Dev Standard: `core/commands/standard-feedback.md`, `FEEDBACK.md`, `core/templates/standard-feedback.md`
- performance: `core/commands/performance-review.md`
- PM lead/project management: `core/commands/pm-log.md`, `core/commands/pm-standup.md`, `core/commands/pm-status.md`, `core/commands/pm-risk.md`, `core/commands/pm-followup.md`, `core/commands/pm-requirement.md`
- AI work event logging: `channels/llm-local/docs/ai-work-event-logging.md`, `channels/llm-local/docs/bda-session-cli.md`, `scripts/bda.mjs`, `scripts/bda-work-event.mjs`
- Hermes/Windsurf/IDE setup: `channels/llm-local/README.md`
- ส่งงาน: `core/commands/handoff-report.md`
...
Adapter notes: Claude Code ใช้ slash commands เฉพาะ interactive mode; Gemini/Claude coworker ใช้ prompt commands; Codex ใช้ agent instruction ใน `channels/llm-local/codex-local/AGENTS.md`.

## Work event logging

งานจริงของ BDA ควรถูกบันทึกเป็น work event เพื่อแทน manual daily log:

- อ่าน `channels/llm-local/docs/ai-work-event-logging.md`
...
- ใช้ `core/commands/test-scenario-report.md`
- ทำตาม `core/workflows/test-scenario-report.md`
- กรอก/สร้างรายงานจาก `core/templates/test-scenario-report.md`
...
ใช้ `core/commands/init.md` เพื่อสำรวจ vault/project และสร้าง `00-Agent-Context.md` จาก `core/templates/obsidian-context.md` พร้อม session/evidence index จาก `core/templates/obsidian-work-note.md`.
...
ใช้เมื่อต้องการปรับปรุง/แก้ไข/เพิ่ม feature ให้ AKS AI Dev Standard เอง เช่น bug/confusion/missing command/feature request/scenario/adoption friction/AI output issue

- ใช้ `core/commands/standard-feedback.md`
- กรอก `core/templates/standard-feedback.md`
- ถ้าต้องแก้มาตรฐาน ให้ใช้ `core/workflows/standard-improvement.md`
- ห้ามผูก feedback นี้กับ performance review, score, KPI, daily performance หรือการประเมินบุคคล

## Output มาตรฐาน

- BDA Standard files used: path ของไฟล์มาตรฐาน BDA ที่ใช้จริง เช่น `STANDARD.md`, `core/commands/...`, `core/workflows/...`, `core/policies/...`, `core/checklists/...`, `core/templates/...`
