#!/bin/bash
# adapter: claude CLI — บทเรียน: CLAUDE_CONFIG_DIR แยกกัน 401 key เก่า, model = bda/deepseek-v4 ชื่อเดียว
set -u
PROMPT="${1:?}"; WD="${2:?}"; LOG="${3:?}"
...
cd "$WD" || exit 1
claude -p "$PROMPT" --model "${AKS_EVAL_CLAUDE_MODEL:-${BDA_EVAL_CLAUDE_MODEL:-bda/deepseek-v4}}" \
